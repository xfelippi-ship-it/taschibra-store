  ...
  return (
    <Link href={`/produto/${p.slug}`}
      ...
      <div className="bg-gray-50 flex items-center justify-center aspect-square w-full overflow-hidden relative">
        ...
        {p.main_image
          ? <img src={getImageUrl(p.main_image, 600)} loading="lazy" alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder-product.svg' }} />
        ...
        <div className="flex flex-col items-center gap-2 w-36">
          <div className="bg-gray-50 border border-gray-200 rounded-xl w-full aspect-square flex items-center justify-center overflow-hidden">
            {produtoAtual.main_image
              ? <img src={getImageUrl(produtoAtual.main_image, 800)} alt={produtoAtual.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder-product.svg' }} />
              : <div className="flex items-center justify-center w-full h-full"><span className="text-5xl">💡</span></div>}
          </div>
            ...
            <div key={p.id} className="flex items-start gap-3">
              ...
              <div className="flex flex-col items-center gap-2 w-36">
                <div onClick={() => setSelecionados(prev => ({ ...prev, [p.id]: !prev[p.id] }))}
                  ...
                  {p.main_image
                    ? <img src={getImageUrl(p.main_image, 600)} loading="lazy" alt={p.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder-product.svg' }} />
                    : <div className="flex items-center justify-center w-full h-full"><span className="text-5xl">💡</span></div>}
                </div>
