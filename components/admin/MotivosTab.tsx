...
const VAZIO: Motivo = { motivo: '', active: true, sort_order: 0 }

export default function MotivosTab() {
  const [itens, setItens] = useState<Motivo[]>([])
  const [loading, setLoading] = useState(true)
  const [modal, setModal] = useState(false)
  const [form, setForm] = useState<Motivo>(VAZIO)
  ...
  async function carregar() {
    setLoading(true)
    const { data } = await (supabase.from as any)('cancel_reasons').select('*').order('sort_order')
    setItens((data || []) as Motivo[])
    setLoading(false)
  }
  ...
  async function salvar() {
    ...
    if (form.id) {
      ...
      ;({ error } = await (supabase.from as any)('cancel_reasons').update(rest).eq('id', id))
    } else {
      ;({ error } = await (supabase.from as any)('cancel_reasons').insert({ ...form, sort_order: itens.length + 1 }))
    }
  ...
  async function excluir(id: string) {
    ...
    await (supabase.from as any)('cancel_reasons').delete().eq('id', id)
    setItens(prev => prev.filter(x => x.id !== id))
    showMsg('Removido!')
  }

  async function toggleAtivo(item: Motivo) {
    await (supabase.from as any)('cancel_reasons').update({ active: !item.active }).eq('id', item.id!)
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-black text-gray-800">Motivos de Cancelamento</h1>
          <p className="text-xs text-gray-400 mt-1">Motivos exibidos ao operador ao cancelar um pedido.</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left px-5 py-3 text-xs font-black text-gray-500 uppercase">Motivo</th>
            ...
            <tbody>
              {itens.map(item => (
                <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-5 py-4 font-bold text-gray-800">{item.motivo}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={() => setModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-black text-gray-800">{form.id ? 'Editar Motivo' : 'Novo Motivo'}</h2>
              <button onClick={() => setModal(false)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-bold text-gray-600 mb-1 block">Motivo *</label>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={salvar} disabled={!form.motivo.trim()}
