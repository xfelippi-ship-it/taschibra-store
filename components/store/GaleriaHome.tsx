...
function BlocoImagem({ item }: { item: GalleryItem }): React.ReactElement {
  const inner = (
...
function BlocoTexto({ item }: { item: GalleryItem }): React.ReactElement {
  const inner = (
    <div className={`relative w-full h-48 md:h-64 rounded-2xl overflow-hidden bg-gradient-to-r ${item.bg_color} flex items-center px-6 md:px-10 group cursor-pointer`}>
      ...
      <div className="relative z-10">
        {item.badge && <span className="inline-block bg-yellow-400 text-yellow-900 text-xs font-black px-2.5 py-1 rounded-full mb-3">{item.badge}</span>}
        {item.titulo && <p className="text-white font-black text-xl md:text-3xl leading-tight mb-1">{item.titulo}</p>}
        {item.subtitulo && <p className="text-white/80 text-sm md:text-base mb-4">{item.subtitulo}</p>}
        {item.btn_label && <span className="inline-block bg-yellow-400 hover:bg-yellow-300 text-yellow-900 font-black text-sm px-5 py-2.5 rounded-full transition-colors">{item.btn_label}</span>}
      </div>
    </div>
  )
...
export default function GaleriaHome() {
  ...
  useEffect(() => {
    ...
    ;(supabase as any)
      ...
      .order('ordem')
      .then(({ data }: any) => setItens((data || []) as GalleryItem[]))
  }, [])

  if (!itens.length) return null

  const cols = itens.length === 1 ? 'grid-cols-1' : itens.length === 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-3'
