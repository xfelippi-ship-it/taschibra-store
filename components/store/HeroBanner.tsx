...
const slides = [
  {
    ...
    btn1: { label: 'Ver Catálogo', href: '/produtos' },
  ...
  },
  {
    ...
    btn1: { label: 'Ver Lâmpadas', href: '/produtos?categoria=lampadas' },
  ...
  },
  {
    ...
    btn1: { label: 'Ver Outlet', href: '/produtos?categoria=outlet' },
          ...
          <div key={i} className={`min-w-full h-full bg-gradient-to-br ${s.bg} relative flex items-center`}>
            ...
            <div className="relative z-10 w-full px-6 md:px-12 max-w-7xl mx-auto">
              <div className="max-w-xl">
                ...
                <p className="text-white/70 text-sm md:text-base leading-relaxed mb-4 md:mb-8 max-w-md hidden sm:block">{s.sub}</p>
                <div className="flex gap-2 md:gap-3">
                  <Link href={s.btn1.href} className="bg-yellow-400 hover:bg-yellow-300 text-black font-black text-xs md:text-sm px-4 md:px-7 py-2.5 md:py-3.5 rounded-lg transition-all">
                    {s.btn1.label}
                  </Link>
                  <Link href={s.btn2.href} className="bg-transparent hover:bg-white/10 text-white font-bold text-xs md:text-sm px-4 md:px-7 py-2.5 md:py-3.5 rounded-lg border border-white/30 transition-all">
                    {s.btn2.label}
                  </Link>
                </div>
              </div>
