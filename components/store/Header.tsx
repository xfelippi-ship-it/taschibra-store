      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-6">
          <a href="/" className="flex-shrink-0">
            <Image src="/images/logo.png" alt="Taschibra Store" width={200} height={52} priority className="h-12 w-auto" />
          </a>
          ...
          <div className="flex items-center gap-4 ml-auto">
            <button className="flex items-center gap-2 text-sm font-semibold text-gray-700 hover:text-green-600 transition-colors">
            ...
            <button className="relative bg-green-600 hover:bg-green-700 text-white font-bold text-sm px-4 py-2.5 rounded-full flex items-center gap-2 transition-colors">
              <ShoppingCart size={18} />
              <span className="hidden md:inline">Carrinho</span>
              <span className="absolute -top-1.5 -right-1.5 bg-yellow-400 text-black text-xs font-black w-5 h-5 rounded-full flex items-center justify-center">0</span>
            </button>
          </div>
        </div>
        <nav className="border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-6 flex items-center overflow-x-auto">
            {['Lançamentos','Exclusivos','SMART','Lâmpadas','Teto','Refletor','Mat. Elétrico','Decorativo','Parede','Perfil','Outlet'].map(item => (
              <a key={item} href="#" className="px-4 py-3 text-sm font-bold text-gray-600 hover:text-green-600 border-b-2 border-transparent hover:border-green-500 transition-all whitespace-nowrap">
                {item}
              </a>
            ))}
          </div>
